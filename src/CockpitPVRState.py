...
class CockpitPVRState():
    ...
    def playStateChanged(self, state):
        ...
        if not self.show_state_pic or (not config.usage.show_infobar_on_skip.value and self.seekstate in (self.SEEK_STATE_PLAY, self.SEEK_STATE_EOF)):
